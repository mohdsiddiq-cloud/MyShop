...
class TinyDB(appContext: Context?) {
    ...
    fun getListObject(key: String?): ArrayList<PopularData> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList: ArrayList<PopularData> = ArrayList<PopularData>()
        for (jObjString in objStrings) {
            val player: PopularData = gson.fromJson(jObjString, PopularData::class.java)
    ...
    fun putListObject(key: String?, playerList: ArrayList<PopularData>) {
        checkForNullKey(key)
        val gson = Gson()
